...
var LocalClient = class {
  ...
  genImagePath(doc, outputDir) {
    ...
    if (this.config.pathFollowDoc) {
      ...
      prefixKey = prefixKey.split(path2.sep).join("/");
    } else {
  ...
  uploadImg(imgBuffer, imageName, doc) {
    return __async(this, null, function* () {
      try {
        ...
        const sysPath = path2.join(prefixKey, imageName);
        return sysPath.split(path2.sep).join("/");
      } catch (e) {
...
var ImageUploader = class {
  ...
  uploadFromFeiShu(urlList, feishuClient, doc, failBack) {
    return __async(this, null, function* () {
      ...
      yield asyncPool(this.config.limit || 3, urlList, promise);
      let output = [];
